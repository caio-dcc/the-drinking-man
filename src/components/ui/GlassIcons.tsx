...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  const getBackgroundStyle = (color: string): React.CSSProperties => {
    // Returning a neutral glass style regardless of the input color
    return {
      background:
  ...
  return (
    <div
      className={`grid gap-[3em] grid-cols-3 mx-auto py-[2em] overflow-visible ${className || ""}`}
    >
      {items.map((item, index) => (
        ...
        >
          ...
          <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-sm font-medium text-white opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(10%)] drop-shadow-md">
            {item.label}
          </span>
        </button>
      ))}
